...
export class SelectExamCourseComponent implements OnInit {
  ...
  constructor(
    ...
    const examDetail = localStorage.getItem(EnumService.localStorageKeys.SELECTED_EXAM_DETAILS);
    if (examDetail) {
      this.examDetail = JSON.parse(examDetail);
      this.prepareExamData();
    }

    this.observablesService.getObservable(EnumService.observalbles.DROP_DOWN_ITEM_SELECT).subscribe((exam) => {
